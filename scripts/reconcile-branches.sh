...
# multipass's entrypoint to the maintain skill's shared namespace script. It
...
if [ ! -f "$script" ]; then
    printf 'multipass branches: the maintain skill is not installed at %s (run ~/code/agentstart/scripts/sync-skills, or set MAINTAIN_SKILL_DIR)\n' \
        "$skill_dir" >&2
    exit 1
fi

export MAINTAIN_WORKSHOP="$(cd "$(dirname "$0")/.." && pwd)"
export MAINTAIN_CHECKOUT="${MULTIPASS_CHECKOUT:-$MAINTAIN_WORKSHOP/fork}"
export MAINTAIN_FORK_REPO=possibilities/codex-multi-auth
export MAINTAIN_UPSTREAM_REPO=ndycode/codex-multi-auth
